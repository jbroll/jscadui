...
export function transpileFunctionCall(
  ...
  // Helper functions from j$ runtime, unless a function declared in the current
  // module shadows the name (dotSCAD's qr_coder defines its own version()).
  // A parameter of the same name does not: NopSCADlib's no_point(str) calls
  // the builtin str(str).
  if (useBuiltin && !ctx.scopes.isDeclaredFunction(callee)) {
