...
- ✅ `echo()` output matches OpenSCAD's line for line (its `-o file.echo` export; warnings are not compared)
- ✅ Uses `openscad --backend=manifold` for watertight geometry

A model whose OpenSCAD top level is empty (it only echoes, like most of
`openscad-tests/scad/functions`) is graded on its echo output alone, and the
transpiled model must draw nothing either. If OpenSCAD stopped on an `ERROR:`
(a failed assert, a recursion limit), the model is not graded. The harness
makes `version()` report the reference OpenSCAD's version.
...
Options:
  ...
  --no-echo               Grade geometry only (models that only echo are not graded)
...
| `apps/jscad-web/examples/openscad/*/skip.txt` | Yes — models that do not render |
| `apps/jscad-web/examples/openscad/*/compare-skip.txt` | Yes — models the STL comparison cannot grade |
| `apps/jscad-web/examples/openscad/*/echo-skip.txt` | Yes — models whose geometry is graded but whose `echo()` output is not compared |
...
- **Skip a failing file**: A model that does not render goes in the category's `skip.txt`, which removes it from the comparison, the browser sweeps, the demo browser and the `ALL.js` grids. A model that renders but that the comparison cannot grade (unseeded `rands()`, a bad reference STL, a missing font) goes in `compare-skip.txt`, which only `test-harness.js` reads. A model whose geometry matches but whose `echo()` output cannot (yet) goes in `echo-skip.txt`, also read only by `test-harness.js`, so its geometry stays graded. All are committed and survive regeneration.
