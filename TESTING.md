...
- **Skip a failing file**: A model that does not render goes in the category's `skip.txt`, which removes it from the comparison, the browser sweeps, the demo browser and the `ALL.js` grids. A model that renders but that the comparison cannot grade (unseeded `rands()`, a bad reference STL, a missing font) goes in `compare-skip.txt`, which only `test-harness.js` reads. Both are committed and survive regeneration.
